...
for i=1:size(sources,1),
    p=p+exp(-((xmat-sources(i,2))/0.1).^2-((ymat-sources(i,1))/0.1).^2);
end
...
for ii=1:nt
 ...
 %if mod(ii,10)==1
 %for i=1:size(sources,1)
  %   p0=p0+noice(ii,i)*exp(-((xmat-sources(i,1))/source_width_x).^2-((ymat-sources(i,2))/source_width_y).^2);
 %end
 %end
...
for m=1:size(mics,1)      
 ...
 if mod(ii,ipauze)==1 || ii == nt
   ...
   hold on 
   plot((nx/xmax)*VOORBEELDEN(:,1),(ny/ymax)*VOORBEELDEN(:,2),'wx','MarkerSize', 30)
   hold on 
   plot((nx/xmax)*5,(ny/ymax)*8,'wx','MarkerSize', 30)
   hold off
   axis equal
   axis on
...
h.FontSize = 30;
 ...
 VERSCHOVEN=zeros(nt,size(mics,1),size(VOORBEELDEN,1));
 AANGEPAST=zeros(nt,size(mics,1),size(VOORBEELDEN,1));
 sum=zeros(nx,ny,nt);
 for m=1:size(mics,1)
        for i=1:nx
            for j=1:ny
                d_m=(sqrt(((nx/xmax)*(mics(m,2))-i)^2 + (((ny/ymax)*mics(m,1))-j)^2));
                versch=circshift(mic(:,m),nt-round(d_m/(c0*(nx/xmax)*dt)));
                verschoven=versch.*sqrt(d_m);
                for k=1:size(VOORBEELDEN,1)
                if i==(nx/xmax)*VOORBEELDEN(k,2) && j==(ny/ymax)*VOORBEELDEN(k,1) 
                    VERSCHOVEN(:,m,k)=versch;
                    AANGEPAST(:,m,k)=verschoven;
                end
                end
                for ii=1:nt
...
AA2=clock;AAA2=['y',int2str(AA(1)),'m',int2str(AA(2)),'d',int2str(AA(3)),'_',int2str(AA(4)),'h',int2str(AA(5))];
 ...
 for ii=1:nt
      ...
      regionalMaxima = imregionalmax(sum(:,:,ii));
   ...
   for r=1:size(rows,1)
       maxim(r)=sum(rows(r),columns(r),ii);
   end
   ...
   %pause
     ...
     [imind,cm] = rgb2ind(frame2im(G),256);
     if image_counter1==1, 
       imwrite(imind,cm,['su_',AAA1,'.gif'],'gif', 'Loopcount',inf); 
       else 
       imwrite(imind,cm,['su_',AAA1,'.gif'],'gif','WriteMode','append'); 
       end;%end if image_counter==1
     %pause(0.5);
     end;%end if imovie==1,
...
h.FontSize = 30;
 ...
 for q=1:size(VOORBEELDEN,1)
     nexttile([1,2])
 for i=1:size(mics,1)
     rec=VERSCHOVEN(:,i,q);
 plot(rec(1:ii),'DisplayName',['(',num2str(mics(i,1)),'m,',num2str(mics(i,2)),'m)'])
   title({['Fase shift at (',num2str(VOORBEELDEN(q,1)),'m,',num2str(VOORBEELDEN(q,2)),'m)']},'FontSize',40);
   xlabel('t (ms)','FontSize',30)
   ylabel("p' (Pa)",'FontSize',30)
   xlim([0 nt]);
   ylim([-0.04 0.06]);
...
h.FontSize = 30;
 ...
 nexttile([1,2])
  for i=1:size(mics,1)
     rec=AANGEPAST(:,i,q);
 plot(rec(1:ii),'DisplayName',['(',num2str(mics(i,1)),'m,',num2str(mics(i,2)),'m)'])
   title({['Fase and amplitude adjustments at (',num2str(VOORBEELDEN(q,1)),'m,',num2str(VOORBEELDEN(q,2)),'m)']},'FontSize',40);
   xlabel('t (ms)','FontSize',30)
   ylabel("p' (Pa)",'FontSize',30)
   xlim([0 nt]);
 h = gca; 
...
%lgd.Layout.Tile = 'east';
    ...
    if imovie==2,
     ...
     [imind,cm] = rgb2ind(frame2im(G),256);
     if image_counter2==1, 
       imwrite(imind,cm,['sum_',AAA2,'.gif'],'gif', 'Loopcount',inf); 
       else 
       imwrite(imind,cm,['sum_',AAA2,'.gif'],'gif','WriteMode','append'); 
       end;%end if image_counter==1
     %pause(0.5);
     end;%end if imovie==1,
